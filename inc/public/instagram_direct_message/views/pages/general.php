...
foreach ($result->getUsers() as $user) {
	$avatars[$user->getPk()] = ig_get_img($this, $user->getProfilePicUrl(), true );
}
?>
...
<form action="<?php _e( get_module_url("send/".$account_id."/".$thread_id) )?>" method="POST" class="actionFormInbox">
	<div class="chatbox">
		<div class="cb-header">
			<div class="cb-info">
				<div class="avatar"><a href="https://www.instagram.com/<?php _e( $thread_title )?>" target="_blank"><img src="<?php _e( ig_get_img($this, $result->getUsers()[0]->getProfilePicUrl() ) )?>"></a></div>
				<div class="desc">
			...
			<div class="cb-wrap-body">
				...
				<div class="cb-item <?php _e ( $viewer_id == $item->getUserId()?"right":"left" )?> thread-item-<?php _e( $item->getItemId() )?>">
					...
					<?php }elseif($item->getItemType() == "media"){?>
						<div class="media">
							<a href="<?php _e( $item->getMedia()->getImageVersions2()->getCandidates()[0]->getUrl() )?>" data-fancybox="group"><img src="<?php _e( ig_get_img($this, $item->getMedia()->getImageVersions2()->getCandidates()[0]->getUrl() ) )?>"></a>
							<div class="time"><?php _e( datetime_show( $item->getTimestamp()/1000000) )?></div>
						</div>
					...
					<?php }elseif($item->getItemType() == "animated_media"){?>
						<div class="media gif">
							<a href="<?php _e( $item->getAnimatedMedia()->getImages()->getFixedHeight()->getUrl() )?>" data-fancybox="group"><img src="<?php _e( ig_get_img($this, $item->getAnimatedMedia()->getImages()->getFixedHeight()->getUrl() ) )?>"></a>
							<div class="time"><?php _e( datetime_show( $item->getTimestamp()/1000000) )?></div>
						</div>
					<?php }?>
				</div>
