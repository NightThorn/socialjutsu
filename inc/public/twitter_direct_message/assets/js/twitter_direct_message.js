"use strict";
function Twitter_direct_message(){
    var self= this;
    this.init= function(){

        self.call_load_more();

        if($(".twitter-direct-message").length > 0){

            if($(".tw-dm-select").length > 0){
                $('.tw-dm-select').vtdropdown();
                $(".twitter-direct-message .option-item").each(function(){
                    var that = $(this);
                    var url = that.data("url");
                    var ids = that.data("ids");

                    that.find("a").addClass("actionItem").attr("href", url).attr("data-content", "tw-dm-list").attr("data-result", "html").attr("data-type-message", "text").attr("data-history", url).attr("data-call-after", "Twitter_direct_message.empty('"+ids+"');");
    ...
    this.empty = function(account_id){
        var action         = PATH+"twitter_direct_message/index/"+account_id+"/null";
    ...
    this.chatbox = function(account_id, thread_id){
       var  h = $(window).height();
        ...
        $(".cb-body").css({"max-height": h - 121 - 70});
        ...
        setInterval(function(){
            ...
            if(media != "" && media != undefined){
                ...
                var that           = $("body");
                var action         = PATH+"twitter_direct_message/send/"+account_id+"/"+thread_id;
            ...
            if($(".cb-body").scrollTop() == 0) {
                if(loading == 0){
                    ...
                    if(cursor != ""){
                        ...
                        $(".cb-loading").show();
                        $.post(PATH+"twitter_direct_message/get_thread/"+account_id+"/"+thread_id, data, function(result){
                            setTimeout(function(){
    ...
    this.ajax_load = function(page){
        var that = $('.instagram-direct-message .column-one');
        var list = that.find(".tw-dm-list");
        var data = that.find(".tw-dm-list .widget-item").last();
    ...
    this.path = function(){
        return PATH+'twitter_direct_message/';
    };
}

Twitter_direct_message= new Twitter_direct_message();
$(function(){
    Twitter_direct_message.init();
});
