<?php
class twitter_analytics extends MY_Controller {
	...
	public function index($ids = "")
	{
		...
		//Sidebar
		$team_id = _t("id");
		$result = $this->model->fetch('*', $this->tb_account_manager, " team_id = '{$team_id}' AND social_network = 'twitter'", "social_network, category", "ASC");
		//End sidebar
	...
	public function cron()
	{
		...
		$actions = $this->model->fetch("*", $this->tb_twitter_analytics, "next_action <= {$time}", "next_action", "ASC", 0, 5);
		if(!$actions){ 
			_e("Empty schedule");
			exit(0);
		}
