...
<div class="recent-activity nicescroll no-update">
	...
	?>
	<a href="https://www.twitter.com/<?php _e( is_numeric($row->media_id)?"post/status/".get_data($row, "media_id"):get_data($data, "username") )?>" target="_blank" class="item-recent-activity wrap-m">
		<div class="info wrap-c">
			<div class="avatar">
				<?php if($data->image){?>
					<img src="<?php _e($data->image)?>">
				<?php }else{?>
					<i class="fab fa-twitter"></i>
				<?php }?>
			</div>
			<div class="detail">
				<div class="desc"><?php _e( twaa($row->action)->text )?></div>
