	...
	<div class="widget-list search-list">
		<?php if (!empty($accounts)) { ?>
			<div class="widget-item widget-item-3 search-list <?php _e(segment(3) == "" ? "active" : "") ?>">
				<a href="<?php _e(get_module_url('index')) ?>" class="actionItem" class="actionItem" data-result="html" data-content="column-two" href="<?php _e(get_module_url('index')) ?>" data-history="<?php _e(get_module_url('index')) ?>" data-call-after="Watermark.init();">
			...
			<?php foreach ($accounts as $row) : ?>
				<div class="widget-item widget-item-3 search-list <?php _e($row->ids == segment(3) ? "active" : "") ?>">
					<a href="<?php _e(get_module_url('index/' . $row->ids)) ?>" class="actionItem" class="actionItem" data-result="html" data-content="column-two" href="<?php _e(get_module_url('index/' . $row->ids)) ?>" data-history="<?php _e(get_module_url('index/' . $row->ids)) ?>" data-call-after="Watermark.init();">
						<div class="icon"><img src="<?php _e(get_url(get_data($row, 'avatar'))) ?>"><img src="<?php _e(get_url(get_data($row, 'img'))) ?>"></div>
						<div class="content content-2">
